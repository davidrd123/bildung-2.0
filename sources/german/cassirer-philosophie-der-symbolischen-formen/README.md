...
- Not yet `texts/cassirer-philosophie-der-symbolischen-formen/` — because no
  ...
  for them.
- Not yet split into `raw/` or `normalized/` — because the present need is
  still localized probing, not sustained close reading.
...
1. verify only the first truly live tranche of Volume I against the PDF
2. leave Volumes II-III localized until they are actually needed
3. only then decide whether a `normalized/` layer is warranted
