#!/usr/bin/env python3
"""Extract a page range from the searchable Google / IA OCR witness PDF."""
...
DEFAULT_PDF = PROJECT_ROOT / "source" / "pdf" / "disquisitio-metaphysica-google-ocr-witness.pdf"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_PDF,
        help="path to the OCR witness PDF",
    )
    parser.add_argument(
        "--output",
        type=str,
        help="output file path; relative paths are resolved from the campaign root",
    )
    parser.add_argument(
        "--mode",
    ...
    command.extend(["-f", str(args.start), "-l", str(args.end), str(args.pdf), "-"])
