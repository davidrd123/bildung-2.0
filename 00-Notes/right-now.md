...
- Current Humboldt priority: keep intake closed unless a source-pressure gap appears. The Cratylus / Ergon-Energeia crosswalk and the first PSF I chapter III / Kawi pressure note now exist as bounded working syntheses. The Pott 1876 / GS VII/1 gap is closed at source-hygiene level: GS VII/1 is now the Cassirer citation witness, while Pott remains a comparison witness with divergent pagination. The GS VI/1 `Ortsadverbien` essay is now landed as raw support for chapter III pronoun/deixis pressure. The exact Kawi pp. 164 ff. / 341 space-verb locus and the Kawi pp. 79-81 `actuale Sein` locus are now normalized. Pott Bd. 2 Sections 3-7 are now landed as a raw run-up extract for Section 8. Kawi Drittes Buch first-section targeted raw routes now cover the Stammverwandtschaft method frame, word-comparison apparatus, and pronoun/number-word comparison for the Aarsleff source-base line. The Wolf letter now has a focused shelf and a normalized `Sprachstudium` excerpt for the PSF I chapter I opening. If Humboldt source-building continues, the only named PSF I source-use gap left is the minor Vaskische Ankündigung. Cassirer's `Kantische Elemente` essay now has an ECW 16 raw witness, so use that before attempting a Humboldt-Kant synthesis and normalize only the exact passages needed.
