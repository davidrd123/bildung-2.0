...
- Current Humboldt priority: keep intake closed unless a source-pressure gap appears. The Cratylus / Ergon-Energeia crosswalk and the first PSF I chapter III / Kawi pressure note now exist as bounded working syntheses. The Pott 1876 / GS VII/1 gap is closed at source-hygiene level: GS VII/1 is now the Cassirer citation witness, while Pott remains a comparison witness with divergent pagination. The exact Kawi pp. 164 ff. / 341 space-verb locus is now normalized, so the next chapter III close-reading target would be Kawi pp. 79-81 on the noun/verb boundary and "actuale Sein"; pull `Ortsadverbien` only if deixis/pronoun pressure becomes central. Cassirer's `Kantische Elemente` essay now has an ECW 16 raw witness, so use that before attempting a Humboldt-Kant synthesis and normalize only the exact passages needed.
