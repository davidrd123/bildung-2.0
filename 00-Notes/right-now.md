...
- **Sources**: Uexküll complete, Anders Band I complete, modern-source shelf active but demand-led. Dormant localized scaffolds now include `Récoltes`, Kant `KdU`, and `Substanzbegriff`; the last is source-near enough for one bounded probe, but still not a live source campaign. Cassirer PSF I-III now have direct volume folders with rough chapter/back-matter files in `sources/german/cassirer-philosophie-der-symbolischen-formen/source/`, plus a web-facing `search-surfaces.yaml`; this is source indexing for guided reading and web-facing search, not a promoted symbolic-forms campaign. The Humboldt shelves now support a bounded PSF I pressure sequence: Cratylus / Ergon-Energeia first, then PSF I chapter III against Kawi.
...
- Current Humboldt priority: keep intake closed unless a source-pressure gap appears. The Cratylus / Ergon-Energeia crosswalk and the first PSF I chapter III / Kawi pressure note now exist as bounded working syntheses. The Pott 1876 / GS VII/1 gap is closed at source-hygiene level: GS VII/1 is now the Cassirer citation witness, while Pott remains a comparison witness with divergent pagination. The GS VI/1 `Ortsadverbien` essay is now landed with two working normalizations for chapter III pronoun/deixis pressure: `020a` pronoun/Ichheit and `020b` Raum/pronominal-deixis. The exact Kawi pp. 164 ff. / 341 space-verb locus, the Kawi Drittes Buch Stammverwandtschaft method opening, and the Kawi pp. 79-81 `actuale Sein` locus are now normalized. Pott Bd. 2 Sections 3-7 are now landed as a raw run-up extract for Section 8. Kawi Drittes Buch first-section targeted raw routes also cover word-comparison apparatus and pronoun/number-word comparison for the Aarsleff source-base line. The Wolf letter now has a focused shelf and a normalized `Sprachstudium` excerpt for the PSF I chapter I opening. The minor Vaskische Ankündigung gap is closed with a GS III shelf and a normalized `Sprache ist ueberall Vermittlerin` / `Ich und Du` excerpt. Cassirer's `Kantische Elemente` essay now has an ECW 16 raw witness plus two working normalizations for Kantian synthesis/objectivity and the Ergon/Energeia conclusion; use those before attempting a Humboldt-Kant synthesis.
