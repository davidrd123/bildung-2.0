...
Purpose: capture source-bound findings from Vol. 2 close translation before they vanish into conversation context or get overpromoted into cross-volume claims.
...
- too source-bound to be filed immediately as cross-volume synthesis

Each entry should stay tied to an actual passage and should prefer local pressure over retrospective framework.

## Current Entries

### 2026-05-13 - Bacon Opening: Experiment Splits Into Mediation And Coercion

Passage: `parts/001-bacon-opening-and-the-critique-of-understanding.md`, printed pages `3-6`.

Cassirer does not introduce Bacon by opposing empiricism to rational construction in a simple way. The first distinction is internal to the appeal to experience and experiment. Leonardo and Kepler treat experiment as the mediator that discloses necessary connections and `Vernunftgründe` within experience itself; Bacon treats nature as a foreign power that must be technically compelled to answer.

Local pressure:

- `Experiment` cannot yet be glossed as one stable term. In the opening contrast it names two opposed epistemic postures.
- `Verstandeskritik` is initially negative and psychological: the point is to restrain the understanding's creative powers and expose sources of error, not yet to ground truth systematically.
- `Geist/geistig` already varies between the mind/nature relation and Renaissance intellectual atmosphere. The English split between `mind` and `intellectual` is provisional.
- Bacon's legal/coercive language matters. `Folterungen`, `Kriminalist`, `Schuldigen`, and `Geheimnis` are not ornamental metaphors; they define the posture toward nature.

Routing:

- Keep as ledger evidence for now.
- Do not add a glossary entry until the next Bacon tranche shows whether `Experiment`, `Verstandeskritik`, or `Geist/geistig` becomes a repeated structural term.

### 2026-05-13 - Bacon: `Erfahrung` As Groping Or Methodical Experiment

Passage: `parts/002-bacon-idols-experience-and-method.md`, printed pages `7-10`.

Cassirer identifies Bacon's real positive contribution as a distinction inside `Erfahrung` itself. Mere experience is passive, accidental groping; genuine experience lights a lamp first, orders observation, draws axioms, and uses those axioms to open new experiments. This confirms that the Part 001 split in `Experiment` was not local ornament but the core of Cassirer's Bacon reading.

Local pressure:

- `Erfahrung` is now structurally doubled: immediate sensory impression versus methodically ordered scientific experience.
- `Experiment` becomes the mediating form through which sense and understanding correct one another.
- `Sinn` is not a safe empirical foundation; Bacon's critique extends to sense as well as to abstract concepts.
- The mirror image recurs as enchanted/uneven mirror, shifting from Bacon's anti-Platonic descent image into a general theory of human distortion.

Routing:

- Keep as ledger evidence for now.
- `Erfahrung` and `Experiment` are close to glossary-entry threshold but should wait for the forms-doctrine transition, because Cassirer has just opened a method/physics tension.

### 2026-05-13 - Bacon: Forms Preserve The Substantial View

Passage: `parts/003-bacon-forms-and-substantial-induction.md`, printed pages `11-17`.

Cassirer uses the opening of `Die Formenlehre` to show why Bacon's method cannot become modern scientific method on its own terms. Bacon can speak of method, experiment, and forms as laws, but his analysis still moves toward thing-properties and simple natures. The decisive contrast is not empiricism versus rationalism; it is relation-concepts versus substantial qualities.

Local pressure:

- `Form` is now a central Bacon term, but it should stay open. It names a self-subsisting quality or simple nature, not a mathematical law-structure.
- `Induktion` is not yet the modern empirical procedure. Baconian induction assumes nature is finite enough to enumerate and exhaust.
- `Tatsache` emerges as a problem: genuine empirical science does not merely receive facts; it uses theoretical criteria to decide what counts as a fact.
- Cassirer names Bacon's doctrine as a Baconian `negative Instanz`, turning Bacon's own methodological vocabulary against him.

Routing:

- Promote `Erfahrung`, `Experiment`, `Form`, and `Induktion` to open glossary entries.
- Keep `Tatsache` in the ledger until Part 004 shows whether it remains structural or only belongs to this local observation/theory contrast.

### 2026-05-14 - Bacon: Forms Become Idols

Passage: `parts/004-bacon-ideas-idols-and-ground-concepts.md`, printed pages `17-21`.

Cassirer turns Bacon's own distinction between idols and ideas against Bacon. Bacon wants ideas to be divine seals impressed into things, but Cassirer argues that the form-concept itself arises from isolating a common mark and projecting that logical result into the object as its inner source.

Local pressure:

- `Form` is no longer only a substantial quality; it is a projected abstraction, hence an idol of the mind.
- `Idole / Ideen` becomes a structural contrast. Bacon's intended distinction fails under Cassirer's own account of human concepts and appearances.
- `idola fori` matters specifically because language supplies the popular divisions that Bacon then relocates into things as opposing forces.
- `Allgemeine / Besondere` is the local name for the contradiction: Bacon attacks abstractions but cannot know particulars without prior universal natures.
- `Selbsttätigkeit` appears in the blank-tablet critique. Part 004 now renders it as `self-activity` to keep the self-prefix visible before later Leibniz/Kant pressure.

Routing:

- Add `Idole / Ideen` to the glossary as open.
- Keep `Allgemeine / Besondere` and `idola fori` under watch until the `philosophia prima` passage shows whether they remain local or chapter-structural.
- Keep `Selbsttätigkeit` as a forward watchpoint rather than promoting it from one occurrence.

### 2026-05-14 - Bacon: `Philosophia Prima` Becomes Rhetorical Generality

Passage: `parts/005-bacon-philosophia-prima-and-rhetorical-generality.md`, printed pages `21-24`.

Bacon's `philosophia prima` looks like it should answer the problem Part 004 exposed: it is supposed to concern relational conditions of knowledge and cross-disciplinary axioms rather than inner grounds of being. Cassirer's judgment is that it fails in execution. Once the place of the universal has already been filled by thing-like qualities, general axioms cannot function constructively; they become rhetorical commonplaces and analogies.

Local pressure:

- `Allgemeine / Besondere` is now stronger than a local Part 004 pair. Part 005 says directly that the place of the `Allgemeine` in knowledge has been occupied by dingliche `Qualitäten`.
- `philosophia prima` is Bacon's attempted alternate route to generality, but Cassirer reads it as a symptom of the same failure rather than a solution.
- Cassirer keeps the double judgment: Bacon heralds the new value of empirical being, but misses the method for mastering the particular.
- The self-reversal pattern has now recurred: Baconian terms and ideals become the measure by which Bacon is found wanting.

Routing:

- Keep this as ledger evidence for now; no new glossary entry.
- If Part 006 turns Bacon's physics or motion theory against Bacon's own methodological demand, create a bounded reading note on Cassirer's critique of Bacon by Baconian standards.
- Watch `Bewegung` as the next likely term-pressure center.

### 2026-05-14 - Bacon: Motion As Appetite, Not Mechanics

Passage: `parts/006-bacon-physics-astronomy-and-motion.md`, printed pages `24-28`.

The closing Bacon tranche tests whether Bacon's turn from being to becoming can escape the substantial view. It cannot. Bacon names motion as the object of physics and asks for a living astronomy that seeks physical grounds rather than merely describing stellar motions, but he still interprets motion through inward strivings, inclinations, hate, love, sympathy, and antipathy.

Local pressure:

- `Bewegung` is now a genuine term-pressure center. The decisive contrast is not simply motion versus rest, but motion as qualitative appetite versus motion as mathematically determinable `Ortsveränderung`.
- `Form` remains under pressure because Bacon tries to transform forms into motion-generating forces without leaving anthropomorphic explanation behind.
- The astronomy passage shows that Copernican acceptance or rejection is not the decisive issue for Cassirer. What matters is Bacon's judgment of the method of modern astronomy.
- The Bacon quotation on daily rotation carries an untranslatable `begriffen` pressure: the heavens or earth is `in täglicher Umdrehung begriffen`, and the nature of spontaneous circular motion must first be `begriffen`. The draft uses separate English verbs, but the German keeps being-in-motion and conceptual grasp in the same verbal field.
- The chapter-closing Atalanta comparison makes the self-reversal pattern explicit: Bacon reaches for the fruits of experience before winning the light-bringing principles that would shape experience scientifically.
