...
These notes collect project-side analogies, external comparisons, and method extrapolations prompted by review.

They are not binding translation decisions and should not outrank the source-bound notes in `journal.md`.
...
- keep recording these method findings when they surface from the translated text itself
...
### Abstractive vs. Constructive: One Cognition at Two Depths

The chapter has now explicitly named the distinction that has been implicit since the introduction: abstractive thinking (isolates fixed oppositions, classifies by similarity, governed by excluded middle) vs. constructive/generative thinking (comprehends the rule that yields an entire series of forms, grasps principle and totality in one act). But Cusanus's own point — and Cassirer is tracking this — is that these are not two separate faculties. They are *one cognition at different depths*. Intellectus is what ratio discovers when it pushes past its own limit. Docta ignorantia is the hinge: reason driven to exhaustion finds its own ground.

This corrects the earlier framing of the dual-translation method. The close translation is not simply "ratio" and the free translation is not simply "intellectus." What happens is: the close translation generates *pressure*. The word-mapping is adequate and dead. The felt gap between adequacy and the living meaning is docta ignorantia — learned ignorance. The free translation emerges *from within that gap*, not from a separate operation applied to the same material. The close version succeeds so completely that it reveals what it can't reach, and that revealing is where the free version is born. This is not switching modes but *one mode discovering its own depth* — exactly as Cusa's ratio, driven to its limit, discovers intellectus as its own ground.

### Subordination and Return (Method Finding)

The ratio/intellectus relation is not equality or replacement. It is subordination and return: *intellectus* discloses the generating form; *ratio* renders it explicit, checks it, makes it communicable. Then the articulated result is reintegrated into the whole from which it came. Seeing → articulation → reintegration. *Assimilatio* names the active process within this cycle — mind making the material answerable to a principle it has grasped.

The relation to McGilchrist is structural homology, not identity. The overlap (part/whole, explicit/implicit, map/presence, exclusion/betweenness) is real and diagnostically productive. But *intellectus* is not "right hemisphere" and *ratio* is not "left hemisphere." Making it one-to-one collapses a productive analogy into a reductive taxonomy — which would be a *ratio* operation applied to a distinction that only *intellectus* can properly hold.

The warning sign: the moment every author gets sorted too quickly into one side, the frame has stopped being a tool of attention and become a tool of classification. The stronger use is diagnostic: where is a text classifying? Where is it generating? How does it move between the two? Gómez Dávila is not "Team Intellectus" — he is a writer whose *ratio* is so sharp that it keeps discovering its own ground. Dick is not "involuntary intellectus" — he is a man whose *ratio* and *intellectus* are in violent oscillation, and the Exegesis is the trace of that oscillation. The frame works as a polarity to be enacted, not as a taxonomy that explains everything in advance.
...
## Review Notes — Part 15 (Scientia Infinita)

### The Presupposition Structure as Method Finding

The quadrature passage ("whoever seeks the quadrature must already presuppose equality between straight and curved") is structurally identical to what happens in translation. The translator who begins work on a sentence must already presuppose that the meaning is translatable — must carry a pre-articulate grasp of the whole that she holds fast against the conflicting indications of the individual words. The *docta ignorantia* moment arrives when she discovers that this presupposition was what made the translation possible all along. Cusa's formula — "what is presupposed in every question is at the same time the light that guides us to what is asked" — is a precise description of how the free translation emerges from within the close translation's pressure. The light was already there; the close version's exhaustive effort is what reveals it.

This connects to the subordination-and-return model: *intellectus* (the presupposed grasp of the whole) is the light that guides *ratio* (the step-by-step articulation) to what it seeks. *Ratio* doesn't produce the light; it discovers it was working by it all along.

### The Mint-Master / Money-Changer Image

The value distinction (God stamps, mind authenticates) is the most precise statement yet of what the glossary practice does. The glossary doesn't *create* the meaning of *Bruchstelle* or *Schicksalszeit*. It *authenticates* renderings — testing them against the source, weighing alternatives, determining which English coin is genuine and which is counterfeit. The substrate (the German meaning) is given; the authentication (the glossary's comparative judgment) is the translator's genuine contribution. This is *assimilatio* applied to the glossary: mind making the material answerable to a principle it has grasped, without pretending to have created the material.
